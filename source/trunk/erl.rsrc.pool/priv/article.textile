...
; {when_exhausted_action, block} : <code>borrow</code> function on exhausted pool is blocked until a new or idle object is available. Waiting time period limited  by value of other option max_wait (see [[#Timing]]).
; {when_exhausted_action, grow}  : <code>borrow</code> function on exhausted pool returns new resource and size of Active Idle grows. In this case max_Idle option is just ignored.
...
<code>resource_factory</code> module defines '''behaivior''' that we have to implement while designing of resource factory module
for given resource.  
